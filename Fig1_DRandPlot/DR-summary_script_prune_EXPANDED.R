...
all10kDR<-fread("DR-matrix_MamPhy_BDvrFIXED_FBD_all10k.txt", header="auto") # 5912 obs. of  10000 variables
...
Count.of.NA<-data.frame(matrix(NA, nrow = 5912, ncol = 1), row.names=rownames(all10kDR))
range<-data.frame(matrix(NA, nrow = 5912, ncol = 1), row.names=rownames(all10kDR))
variance<-data.frame(matrix(NA, nrow = 5912, ncol = 1), row.names=rownames(all10kDR))
stdev<-data.frame(matrix(NA, nrow = 5912, ncol = 1), row.names=rownames(all10kDR))
cv<-data.frame(matrix(NA, nrow = 5912, ncol = 1), row.names=rownames(all10kDR))
sterror<-data.frame(matrix(NA, nrow = 5912, ncol = 1), row.names=rownames(all10kDR))
...
write.table(all10kDR_summary, "DR-SUMMARY_MamPhy_BDvrFIXED_FBD_all10k_expanded.txt", col.names=TRUE, row.names=TRUE)


###
all10kDR<-fread("DR-matrix_MamPhy_BDvrFIXED_NDexp_all10k.txt", header="auto") # 5912 obs. of  10000 variables
...
write.table(all10kDR_summary, "DR-SUMMARY_MamPhy_BDvrFIXED_NDexp_all10k_expanded.txt", col.names=TRUE, row.names=TRUE)
