...
write.table(g_es_dr, "DR-matrix_MamPhy_YuleCR_all10ktrees.txt")

pdf(file = "DRplot_5910spp.pdf", height=8, width=8)
title("DR - MamPhy 5910 spp, Yule CR, 10k trees", cex=1.5, font=2)
hist(unlist(g_es_dr), breaks=1000)
dev.off()


####
# Now as with tips DROPPED
##
...
trees_drop = foreach(i in 1:length(trees)) %dopar% {
...
write.table(g_es_dr_DROP, "DR-matrix_MamPhy_YuleCR_all10ktrees_pruned4098.txt")

pdf(file = "DRplot_4098spp.pdf", height=8, width=8)
title("DR - MamPhy 4098 spp, Yule CR, 10k trees", cex=1.5, font=2)
