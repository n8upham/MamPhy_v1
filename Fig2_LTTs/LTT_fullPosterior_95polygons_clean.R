...
placentals<-setdiff(mamPhy_100[[1]]$tip.label,marMono)
placMono<-c(as.vector(placentals),as.vector(monotremes))

mamPhy_100_placentals<-vector("list",length(mamPhy_100))
for (i in 1:length(mamPhy_100_placentals)){
	mamPhy_100_placentals[[i]]<-drop.tip(mamPhy_100[[i]],marMono)
}
write.nexus(mamPhy_100_placentals, file=paste("MamPhy_fullPosterior_BDvr_pcsFIXED_",bbone,"_sample100_PLACENTALS.trees",sep=""))

mamPhy_100_marsupials<-vector("list",length(mamPhy_100))
for (i in 1:length(mamPhy_100_marsupials)){
	mamPhy_100_marsupials[[i]]<-drop.tip(mamPhy_100[[i]],placMono)
}
write.nexus(mamPhy_100_marsupials, file=paste("MamPhy_fullPosterior_BDvr_pcsFIXED_",bbone,"_sample100_MARSUPIALS.trees",sep=""))
