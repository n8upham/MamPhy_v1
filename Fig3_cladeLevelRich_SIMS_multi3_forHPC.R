...
# ========================
slicePhys<-read.tree(file=paste("MamPhy_SIMS_",sims[z],"_",bbone,"_tree_",i,"_CORRECT_slicePhy-5to70Ma_by5.trees",sep=""))
...
for (q in 1: length(allCladeSetNames)){
	# Load in
	res<-read.table(paste("MamPhy_SIMS_",sims[z],"_",bbone,"_sample100_",i,"_slice",allCladeSetNames[[q]],"_cladeSTATS_Age-n-Rate_andSingletons.txt",sep=""), header=TRUE)
	# select RESPONSE vars
	RESP<-log(res[,"richness"])
	# select PREDICTOR vars
	PRED<-res[,c(1:2,4,6:11)] # excluding DR-KURTOSIS, excluding the BD.ms vars
...
#		#cat(paste("tree_",i, ", sim ",sims[z],", slice_", j, ", predictor ",predictors[k],sep=""))
#
...
#	write.table(uniPGLS,paste("MamPhy_SIMS_",sims[z],"_",bbone,"_sample100_",i,"_PGLS_",corr,"_explaining-Richness_perSlice_",sliceN,"_SCALED_uniVar_Correct-rootwardWithSingle_9preds.txt",sep=""))
...
# 3 var
	...
	write.table(multiPGLS,paste("MamPhy_SIMS_",sims[z],"_",bbone,"_sample100_",i,"_PGLS_",corr,"_explaining-Richness_perSlice_",sliceN,"_SCALED_multiVar_Correct-rootwardWithSingle_age-DRmean-DRskew.txt",sep=""))
