...
for (q in 1: length(allCladeSetNames)){
	...
	PRED_LOGvars<-colnames(head(res,0)[c(11:12,15,18,21,24:25,29,35)]) # exclude richness
	PRED_NONvars<-colnames(head(res,0)[c(3,13:14)]) # "DR_harm","DR_skew","BD_Div"
	...
	colnames(results[[q]])<-c("logTipDR","logBD_Div","logMRCA","DRskew",paste("log",(PRED_LOGvars),sep=""),(PRED_NONvars))
}
...
#predictors<-c(predictorsALL[c(1:25)]) # focusing on continuous vars, trophic vars
...
#		fit1<-gls(form, correlation=corBrownian(phy=cladeData$phy), data=dat, method="ML")
	...
	write.table(uniPGLS,paste(bbone,"_sample100_",i,"_PGLS_",corr,"_explaining-DR-MRCA_perSlice_",sliceN,"_SCALED_uniVar_Correct-rootwardWithSingle_12preds.txt",sep=""))
