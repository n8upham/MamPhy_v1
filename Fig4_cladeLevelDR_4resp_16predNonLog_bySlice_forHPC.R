...
# - Run for loop of 4 response vars 
# - Setup empty dataframes to receive PGLS results 
# - Nest the loop of 16 predictor vars
...
for (q in 1: length(allCladeSetNames)){
	...
	PRED_NONvars<-colnames(head(res,0)[c(3,5,15,18,20:26)]) # # focusing on continuous vars, trophic vars
#	PRED_all<-cbind(log(res[,PRED_LOGvars]), res[,PRED_NONvars])
	PRED_all<-cbind(res[,PRED_LOGvars], res[,PRED_NONvars]) # so NOT LOGGING ANY VARS -- see how this does.
	...
	results[[q]]<-cbind(log(RESP[,1:3]),RESP[,4],predScale[,1:length(PRED_all[1,])])
	colnames(results[[q]])<-c("logTipDR","logBD_Div","logRichness","DR_skew",colnames(PRED_all))
}
...
#		fit1<-gls(form, correlation=corBrownian(phy=cladeData$phy), data=dat, method="ML")
	...
	write.table(uniPGLS,paste(bbone,"_sample100_",i,"_PGLS_",corr,"_explaining-4varDR_perSlice_",sliceN,"_SCALED_uniVar_Correct-rootwardWithSingle_16predsNON-LOG.txt",sep=""))
