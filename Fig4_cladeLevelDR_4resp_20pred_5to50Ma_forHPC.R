...
for (q in 1: length(allCladeSetNames)){
	...
	PRED_LOGvars<-c(colnames(head(res,0)[c(11:13,16,19)]),"richness")
	PRED_NONvars<-c("DR_harm","DR_skew","BD_Div",colnames(head(res,0)[c(3,5,15,18,20:36)]))
	...
	colnames(results[[q]])<-c("logTipDR","logBD_Div","logRichness","DRskew",paste("log",PRED_LOGvars,sep=""),PRED_NONvars)
}
...
predictors<-c(predictorsALL[c(1:20)])
...
# ===============================
	for (j in 1:10){ # do for 5 Ma to 50 Ma slices
...
#corr<-"BROWNIAN"
corr<-"PAGEL"
sliceN<-"5to50Ma" #"ALL" #"45Ma-only" #"10to60Ma" # # #"35Ma-only" ##"all" #"60Ma"#"all" #

uniPGLS<-cbind(uniPGLS_allInts,uniPGLS_allSlopes,uniPGLS_allPs,uniPGLS_allSEs,uniPGLS_allLams)
write.table(uniPGLS,paste(bbone,"_sample100_",i,"_PGLS_",corr,"_explaining-",respVar[z],"_timeSlices_",sliceN,"_SCALED_uniVar_Correct_20preds.txt",sep=""))
