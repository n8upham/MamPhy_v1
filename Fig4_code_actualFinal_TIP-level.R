...
predictors<-c("genes",varsSelected)
perPredictor<-vector("list",length(predictors))
	...
	form<-as.formula(paste("log(tipDR) ~ ", predictors[j],sep=""))
...
allRes_i<-cbind.data.frame(do.call(rbind,perPredictor),rep(i,length(predictors)),rep(catsSelected[z],length(predictors)))
...
write.table(allRes_i,file=paste("speciesLevel_PGLS-PAGEL_LogTipDR-vs-NONLog-EcoGeoTraits_",GROUP,"_tree",i,".txt",sep=""))
...
# =======================
# SUMMARIZE those PGLS results now
# =======================
