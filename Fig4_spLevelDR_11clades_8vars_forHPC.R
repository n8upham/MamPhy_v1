...
ordsSelected<-c("DIDELPHIMORPHIA", "DIPROTODONTIA", "CARNIVORA", "CETARTIODACTYLA", "PRIMATES", "EULIPOTYPHLA", "CHIROPTERA", "RODENTIA") 
higherSelected<-c("Marsupialia", "Laurasiatheria", "Euarchontoglires")
taxaSelected<-c(ordsSelected,higherSelected)

allDatFinal<-cbind.data.frame(tipDR_i,taxFinal,tipDataAll[,varsSelected],tipDataAll[,catsSelected])
# if doing GLOBAL
#datFinal<-allDatFinal 

# if doing TAXON SUBSETS
for(z in 1:length(taxaSelected)){

if(z < 9){
datFinal<-allDatFinal[which(allDatFinal[,"ord"]==taxaSelected[z]),]
} else {
datFinal<-allDatFinal[which(allDatFinal[,"higher"]==taxaSelected[z]),]
}
...
allRes_i<-cbind.data.frame(do.call(rbind,perPredictor),rep(i,length(varsSelected2)),rep(taxaSelected[z],length(varsSelected2)))
