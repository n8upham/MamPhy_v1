...
# start parallel loop
ntrees=100
foreach(i=ntrees:1, .packages=c('geiger','moments', 'nlme', 'ape'), .combine=cbind, .verbose=TRUE) %dopar% {
...
for(j in 4:length(varsSelected2)){
...
#	fit1<-possibleError
	fit1<-gls(form, correlation=corPagel(value=0.5,phy=treeDat$phy), data=datPlot, method="ML")
...
#	lambda=NA
	...
	perPredictor[[j]]<-cbind.data.frame(a,b,SE,pVal,lambda,var,round(i,0))
	write.table(perPredictor[[j]],file=paste("speciesLevel_PGLS-PAGEL_LogTipDR-vs-Log-",var,"_",catsSelected[z],"_tree",i,".txt",sep=""))
}
