...
PRED_LOGvars<-c("homeRange_km2_ext","trophic123", "geoArea_km2", "GenerationLength_d") # "BM_final_kg", "DispDistAll_km_ext",
...
allDatFinal<-cbind.data.frame(taxFinal,log(tipDR_i),log(tipDataAll[,PRED_LOGvars]),tipDataAll[,PRED_NONvars],(taxFinal$genes),tipDataAll[,catsSelected])
colnames(allDatFinal)<-c(taxonomyVars,"logTipDR",PRED_names,catsSelected)
...
# #    -0.07995504     0.04806887     0.21767717 
		...
		form<-as.formula(paste(respVar, " ~ ", predictors[1], "+", predictors[2], "+", predictors[3], "+", predictors[4], "+", predictors[5], "+", predictors[6], "+", predictors[7], sep="")) #"+", predictors[8], 
		...
		form<-as.formula(paste(respVar, " ~ ", predictors[1], "+", predictors[2], "+", predictors[3], "+", predictors[4], "+", predictors[5], "+", predictors[7], sep=""))
		model_6<-pgls(form, data=cladeData, lambda=1.0)

		form<-as.formula(paste(respVar, " ~ ", predictors[1], "+", predictors[2], "+", predictors[3], "+", predictors[4], "+", predictors[6], "+", predictors[7], sep=""))
		model_5<-pgls(form, data=cladeData, lambda=1.0)

		form<-as.formula(paste(respVar, " ~ ", predictors[1], "+", predictors[2], "+", predictors[3], "+", predictors[5], "+", predictors[6], "+", predictors[7], sep=""))
		model_4<-pgls(form, data=cladeData, lambda=1.0)

		form<-as.formula(paste(respVar, " ~ ", predictors[1], "+", predictors[2], "+", predictors[4], "+", predictors[5], "+", predictors[6], "+", predictors[7], sep=""))
		model_3<-pgls(form, data=cladeData, lambda=1.0)

		form<-as.formula(paste(respVar, " ~ ", predictors[1], "+", predictors[3], "+", predictors[4], "+", predictors[5], "+", predictors[6], "+", predictors[7], sep=""))
		model_2<-pgls(form, data=cladeData, lambda=1.0)

		form<-as.formula(paste(respVar, " ~ ", predictors[2], "+", predictors[3], "+", predictors[4], "+", predictors[5], "+", predictors[6], "+", predictors[7], sep=""))
		model_1<-pgls(form, data=cladeData, lambda=1.0)

		anovaCompare<-as.data.frame(anova.pgls(model_ALL, model_1, model_2, model_3, model_4, model_5, model_6, model_7))
		aicCompare<-AIC(model_ALL, model_1, model_2, model_3, model_4, model_5, model_6, model_7)
	...
	write.table(fullResALL, file=paste("tipLevel",bbone,"_sample100_",i,"_PGLS_",corr,"_explaining-tipDR_multiANOVA_7preds.txt",sep=""))
